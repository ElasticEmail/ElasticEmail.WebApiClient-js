function EEAPI(options) {
     ...
     * @param {Number} maxContacts - Maximum number of contacts the account can have
     ...
     * @return {String}
     */
    account.AddSubAccount = function (email, password, confirmPassword, requiresEmailCredits, enableLitmusTest, requiresLitmusCredits, maxContacts, enablePrivateIPRequest, sendActivation, returnUrl, sendingPermission, enableContactFeatures, poolName, emailSizeLimit, dailySendLimit, callback) {
     ...
     * @return {String}
     */
    account.ChangeEmail = function (newEmail, confirmEmail, sourceUrl, callback) {
        sourceUrl = typeof sourceUrl !== 'undefined' ? sourceUrl : 'https://elasticemail.com/account/';
        request('/account/changeemail', {newEmail: newEmail, confirmEmail: confirmEmail, sourceUrl: sourceUrl}, callback, 'POST');
    };
     ...
     * @param {Boolean} webNotificationNotifyOncePerEmail - True, if you want to receive notifications for each type only once per email. Otherwise, false
     ...
     * @param {Number} staleContactScore - (0 means this functionality is NOT enabled) Score, depending on the number of times you have sent to a recipient, at which the given recipient should be moved to the Stale status
     * @param {Number} staleContactInactiveDays - (0 means this functionality is NOT enabled) Number of days of inactivity for a contact after which the given recipient should be moved to the Stale status
     * @param {String} deliveryReason - Why your clients are receiving your emails.
     * @param {Boolean} tutorialsEnabled - 
     * @param {Function} callback
     * @return {AdvancedOptions}
     */
    account.UpdateAdvancedOptions = function (enableClickTracking, enableLinkClickTracking, manageSubscriptions, manageSubscribedOnly, transactionalOnUnsubscribe, skipListUnsubscribe, autoTextFromHtml, allowCustomHeaders, bccEmail, contentTransferEncoding, emailNotificationForError, emailNotificationEmail, webNotificationUrl, webNotificationNotifyOncePerEmail, webNotificationForSent, webNotificationForOpened, webNotificationForClicked, webNotificationForUnsubscribed, webNotificationForAbuseReport, webNotificationForError, hubCallBackUrl, inboundDomain, inboundContactsOnly, lowCreditNotification, enableUITooltips, enableContactFeatures, notificationsEmails, unsubscribeNotificationsEmails, logoUrl, enableTemplateScripting, staleContactScore, staleContactInactiveDays, deliveryReason, tutorialsEnabled, callback) {
        ...
        webNotificationNotifyOncePerEmail = typeof webNotificationNotifyOncePerEmail !== 'undefined' ? webNotificationNotifyOncePerEmail : null;
        ...
        hubCallBackUrl = typeof hubCallBackUrl !== 'undefined' ? hubCallBackUrl : '';
     ...
     * @param {Boolean} notifyOncePerEmail - True, if you want to receive notifications for each type only once per email. Otherwise, false
     * @param {String} settings - Http notification settings serialized to JSON 
     * @param {Function} callback
     */
    account.UpdateHttpNotification = function (url, notifyOncePerEmail, settings, callback) {
        notifyOncePerEmail = typeof notifyOncePerEmail !== 'undefined' ? notifyOncePerEmail : false;
        settings = typeof settings !== 'undefined' ? settings : null;
        request('/account/updatehttpnotification', {url: url, notifyOncePerEmail: notifyOncePerEmail, settings: settings}, callback, 'POST');
    };
     ...
     * @param {Boolean} marketingConsent - True if you want to receive newsletters from Elastic Email. Otherwise, false. Empty to leave the current value.
     ...
     * @param {Function} callback
     */
    account.UpdateProfile = function (firstName, lastName, address1, city, state, zip, countryID, marketingConsent, address2, company, website, logoUrl, taxCode, phone, callback) {
        marketingConsent = typeof marketingConsent !== 'undefined' ? marketingConsent : null;
        ...
        request('/account/updateprofile', {firstName: firstName, lastName: lastName, address1: address1, city: city, state: state, zip: zip, countryID: countryID, marketingConsent: marketingConsent, address2: address2, company: company, website: website, logoUrl: logoUrl, taxCode: taxCode, phone: phone}, callback, 'POST');
    };
     ...
     */
    attachment.Get = function (attachmentID, callback) {

        request('/attachment/get', {attachmentID: attachmentID}, callback, 'POST');
    };
    ...
    contact.Add = function (publicAccountID, email, publicListID, listName, title, firstName, lastName, phone, mobileNumber, notes, gender, birthDate, city, state, postalCode, country, organizationName, website, annualRevenue, industry, numberOfEmployees, source, returnUrl, sourceUrl, activationReturnUrl, activationTemplate, sendActivation, consentDate, consentIP, field, notifyEmail, callback) {
        ...
        numberOfEmployees = typeof numberOfEmployees !== 'undefined' ? numberOfEmployees : null;
     ...
     * @param {Array.<ContactStatus>} statuses - List of blocked statuses: Abuse, Bounced or Unsubscribed
     * @param {String} search - Text fragment used for searching.
     * @param {Number} limit - Maximum of loaded items.
     * @param {Number} offset - How many items should be loaded ahead.
     * @param {Function} callback
     ...
     * @return {String}
     */
    contact.Subscribe = function (publicAccountID, callback) {

        request('/contact/subscribe', {publicAccountID: publicAccountID}, callback, 'POST');
    };
     ...
     * @param {Boolean} allowUnsubscribe - True: Allow unsubscribing from this (optional) newly created list. Otherwise, false
     ...
     * @return {Number}
     */
    contact.Upload = function (contactFile, allowUnsubscribe, listID, listName, status, consentDate, consentIP, callback) {
        allowUnsubscribe = typeof allowUnsubscribe !== 'undefined' ? allowUnsubscribe : false;
        listID = typeof listID !== 'undefined' ? listID : null;
        listName = typeof listName !== 'undefined' ? listName : null;
        status = typeof status !== 'undefined' ? status : 'active';
        consentDate = typeof consentDate !== 'undefined' ? consentDate : null;
        consentIP = typeof consentIP !== 'undefined' ? consentIP : null;
        uploadPostFile('/contact/upload', contactFile, {allowUnsubscribe: allowUnsubscribe, listID: listID, listName: listName, status: status, consentDate: consentDate, consentIP: consentIP}, callback);
    };
     ...
     * @param {Function} callback
     */
    domain.Add = function (domain, trackingType, callback) {
        trackingType = typeof trackingType !== 'undefined' ? trackingType : 'http';
        request('/domain/add', {domain: domain, trackingType: trackingType}, callback, 'POST');
    };
     ...
     * @param {Function} callback
     */
    domain.VerifyTracking = function (domain, trackingType, callback) {
        trackingType = typeof trackingType !== 'undefined' ? trackingType : 'http';
        request('/domain/verifytracking', {domain: domain, trackingType: trackingType}, callback, 'POST');
    };
     ...
     */
    email.GetStatus = function (transactionID, showFailed, showSent, showDelivered, showPending, showOpened, showClicked, showAbuse, showUnsubscribed, showErrors, showMessageIDs, callback) {
        showFailed = typeof showFailed !== 'undefined' ? showFailed : false;
        showSent = typeof showSent !== 'undefined' ? showSent : false;
     ...
     * @param {Array.<LogJobStatus>} statuses - List of comma separated message statuses: 0 for all, 1 for ReadyToSend, 2 for InProgress, 4 for Bounced, 5 for Sent, 6 for Opened, 7 for Clicked, 8 for Unsubscribed, 9 for Abuse Report
     * @param {ExportFileFormats} fileFormat - Format of the exported file
     ...
     * @param {Array.<MessageCategory>} messageCategory - ID of message category
     * @param {String} email - Proper email address.
     * @param {Boolean} useStatusChangeDate - True, if 'from' and 'to' parameters should resolve to the Status Change date. To resolve to the creation date - false
     * @param {Function} callback
     * @return {Log}
     */
    log.Load = function (statuses, from, to, channelName, limit, offset, includeEmail, includeSms, messageCategory, email, useStatusChangeDate, callback) {
        from = typeof from !== 'undefined' ? from : null;
        to = typeof to !== 'undefined' ? to : null;
        channelName = typeof channelName !== 'undefined' ? channelName : null;
        limit = typeof limit !== 'undefined' ? limit : 0;
        ...
        messageCategory = typeof messageCategory !== 'undefined' ? messageCategory : null;
        email = typeof email !== 'undefined' ? email : null;
        useStatusChangeDate = typeof useStatusChangeDate !== 'undefined' ? useStatusChangeDate : false;
        request('/log/load', {statuses: statuses, from: from, to: to, channelName: channelName, limit: limit, offset: offset, includeEmail: includeEmail, includeSms: includeSms, messageCategory: messageCategory, email: email, useStatusChangeDate: useStatusChangeDate}, callback, 'POST');
    };
     ...
     * @return {Log}
     */
    log.LoadNotifications = function (statuses, from, to, limit, offset, messageCategory, useStatusChangeDate, notificationType, callback) {
        from = typeof from !== 'undefined' ? from : null;
        to = typeof to !== 'undefined' ? to : null;
        limit = typeof limit !== 'undefined' ? limit : 0;
        offset = typeof offset !== 'undefined' ? offset : 0;
        messageCategory = typeof messageCategory !== 'undefined' ? messageCategory : null;
        useStatusChangeDate = typeof useStatusChangeDate !== 'undefined' ? useStatusChangeDate : false;
        notificationType = typeof notificationType !== 'undefined' ? notificationType : 'all';
        request('/log/loadnotifications', {statuses: statuses, from: from, to: to, limit: limit, offset: offset, messageCategory: messageCategory, useStatusChangeDate: useStatusChangeDate, notificationType: notificationType}, callback, 'POST');
    };
     ...
     * @param {Date} to - Ending date for search in YYYY-MM-DDThh:mm:ss format.
     * @param {String} channelName - Name of selected channel.
     * @param {IntervalType} interval - 'Hourly' for detailed information, 'summary' for daily overview
     * @param {String} transactionID - ID number of transaction
     * @param {Function} callback
     * @return {LogSummary}
     */
     ...
     * @param {Date} from - From what date should the segment history be shown. In YYYY-MM-DDThh:mm:ss format.
     * @param {Date} to - To what date should the segment history be shown. In YYYY-MM-DDThh:mm:ss format.
     * @param {Function} callback
